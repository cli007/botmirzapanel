...
class ManagePanel{
    ...
    function createUser($name_panel,$usernameC, array $Data_Config){
        ...
        elseif($Get_Data_Panel['type'] == "x-ui_single"){
            ...
            $Output['subscription_url'] = $domain[0].":".$domain[1].":2082/5u8/{$subId}?name=$subId";
        ...
        elseif($Get_Data_Panel['type'] == "x-ui_single"){
           ...
           $UsernameData2 = get_clinets($username,$Get_Data_Panel['name_panel']);
            ...
            $Output = array(
                ...
                'links' => [outputlunk($domain[0].":".$domain[1].":2082/5u8/{$UsernameData2['subId']}?name={$UsernameData2['subId']}")],
                'subscription_url' => $domain[0].":".$domain[1].":2082/5u8/{$UsernameData2['subId']}?name={$UsernameData2['subId']}",
                );
            }
        }
        else{
    ...
    function Revoke_sub($name_panel,$username){
        ...
        elseif($Get_Data_Panel['type'] == "x-ui_single"){
           ...
           $updateinbound = updateClient($Get_Data_Panel['name_panel'],$username,$config);
            ...
            $Output = array(
                'status' => 'successful',
                'configs' => outputlunk($domain[0].":".$domain[1].":2082/5u8/{$subId}?name=$subId"),
                'subscription_url' => $domain[0].":".$domain[1].":2082/5u8/{$subId}?name=$subId",
                );
            }
        }
